const fa = {
  app: {
    name: 'سیستم جمع‌آوری اطلاعات',
    shortName: 'داده‌برداری'
  },
  nav: {
    dashboard: 'داشبورد',
    collect: 'جمع‌آوری داده',
    records: 'سوابق',
    settings: 'تنظیمات',
    admin: 'مدیریت'
  },
  nfc: {
    title: 'شناسایی دستگاه',
    startScan: 'شروع اسکن NFC',
    stopScan: 'توقف اسکن',
    scanning: 'در حال اسکن...',
    waitingForTag: 'تگ NFC را به دستگاه نزدیک کنید',
    tagDetected: 'تگ شناسایی شد',
    notSupported: 'NFC در این دستگاه پشتیبانی نمی‌شود',
    permissionDenied: 'دسترسی به NFC رد شد',
    error: 'خطا در خواندن تگ NFC',
    serialNumber: 'شناسه تگ',
    manualEntry: 'ورود دستی شناسه',
    manualEntryDisabled: 'ورود دستی در تنظیمات غیرفعال است',
    continueWithTag: 'ادامه با این تگ',
    tagNotRegistered: 'این تگ در سیستم ثبت نشده است. ابتدا آن را در بخش مدیریت ثبت کنید.',
    tagLookupError: 'خطا در بارگذاری اطلاعات دستگاه'
  },
  collect: {
    title: 'جمع‌آوری میدانی داده',
    stepTag: 'شناسایی دستگاه',
    stepForm: 'ورود اطلاعات',
    assetInfo: 'اطلاعات دستگاه',
    assetType: 'نوع دستگاه',
    location: 'محل نصب'
  },
  record: {
    draft: 'پیش‌نویس',
    approved: 'تأیید شده',
    saveDraft: 'ذخیره پیش‌نویس',
    approveAndSend: 'تأیید و ارسال',
    approve: 'تأیید',
    approveConfirm: 'این رکورد تأیید و در صف ارسال قرار می‌گیرد. ادامه می‌دهید؟',
    editRecord: 'ویرایش رکورد',
    viewRecord: 'مشاهده رکورد',
    fieldValues: 'داده‌های ثبت‌شده',
    all: 'همه',
    drafts: 'پیش‌نویس‌ها',
    approved_pl: 'تأیید شده‌ها'
  },
  form: {
    cancel: 'انصراف',
    required: 'این فیلد الزامی است',
    notes: 'یادداشت',
    operatorName: 'نام اپراتور',
    location: 'محل',
    success: 'اطلاعات با موفقیت ذخیره شد',
    draftSaved: 'پیش‌نویس ذخیره شد',
    approvedAndQueued: 'تأیید شد و در صف ارسال قرار گرفت',
    error: 'خطا در ذخیره اطلاعات',
    saving: 'در حال ذخیره...'
  },
  admin: {
    title: 'مدیریت',
    assetTypes: 'انواع دستگاه',
    assetRegistry: 'دفتر دستگاه‌ها',
    addAssetType: 'افزودن نوع دستگاه',
    editAssetType: 'ویرایش نوع دستگاه',
    deleteAssetType: 'حذف نوع دستگاه',
    assetTypeName: 'نام نوع دستگاه',
    noAssetTypes: 'هیچ نوع دستگاهی تعریف نشده',
    noAssetEntries: 'هیچ دستگاهی ثبت نشده',
    addField: 'افزودن فیلد',
    editField: 'ویرایش فیلد',
    fieldKey: 'کلید (انگلیسی)',
    fieldLabel: 'عنوان فارسی',
    fieldType: 'نوع فیلد',
    fieldRequired: 'اجباری',
    fieldUnit: 'واحد (اختیاری)',
    fieldMin: 'حداقل',
    fieldMax: 'حداکثر',
    fieldOptions: 'گزینه‌ها (هر خط یک گزینه)',
    fieldTypes: {
      text: 'متن',
      number: 'عدد',
      select: 'انتخابی (تک)',
      multiselect: 'انتخابی (چند)',
      checkbox: 'چک‌باکس',
      textarea: 'متن بلند'
    },
    addAssetEntry: 'ثبت دستگاه جدید',
    editAssetEntry: 'ویرایش دستگاه',
    tagId: 'شناسه تگ NFC',
    assetName: 'نام دستگاه',
    assetType: 'نوع دستگاه',
    location: 'محل نصب (اختیاری)',
    deleteConfirm: 'آیا از حذف مطمئن هستید؟',
    fieldsCount: 'فیلد',
    selectAssetType: 'نوع دستگاه را انتخاب کنید',
    tagIdHelper: 'شناسه باید منحصربه‌فرد باشد'
  },
  sync: {
    pending: 'در انتظار ارسال',
    syncing: 'در حال همگام‌سازی...',
    synced: 'همگام‌سازی شد',
    failed: 'خطا در همگام‌سازی',
    lastSync: 'آخرین همگام‌سازی',
    pendingCount: '{{count}} رکورد در انتظار',
    manualSync: 'همگام‌سازی دستی',
    online: 'آنلاین',
    offline: 'آفلاین'
  },
  records: {
    title: 'سوابق',
    empty: 'هیچ رکوردی ثبت نشده است',
    emptyDrafts: 'هیچ پیش‌نویسی وجود ندارد',
    emptyApproved: 'هیچ رکورد تأیید شده‌ای وجود ندارد',
    createdAt: 'تاریخ ثبت',
    updatedAt: 'آخرین ویرایش',
    status: 'وضعیت',
    delete: 'حذف',
    deleteConfirm: 'آیا از حذف این رکورد مطمئن هستید؟',
    yes: 'بله',
    no: 'خیر'
  },
  settings: {
    title: 'تنظیمات',
    connection: 'اتصال به سرور',
    operatorSection: 'اطلاعات اپراتور',
    nfcSection: 'تنظیمات NFC',
    serverUrl: 'آدرس سرور',
    syncInterval: 'فاصله همگام‌سازی (ثانیه)',
    operatorName: 'نام اپراتور',
    locationName: 'نام محل',
    allowManualEntry: 'اجازه ورود دستی شناسه تگ',
    allowManualEntryHint: 'اگر فعال باشد، کاربر می‌تواند شناسه را بدون اسکن NFC وارد کند',
    save: 'ذخیره',
    saved: 'تنظیمات ذخیره شد'
  },
  dashboard: {
    title: 'داشبورد',
    totalRecords: 'کل رکوردها',
    pendingSync: 'در انتظار ارسال',
    todayRecords: 'ثبت‌شده امروز',
    quickCollect: 'جمع‌آوری داده',
    drafts: 'پیش‌نویس'
  },
  common: {
    loading: 'در حال بارگذاری...',
    error: 'خطایی رخ داد',
    retry: 'تلاش مجدد',
    close: 'بستن',
    confirm: 'تأیید',
    cancel: 'انصراف',
    save: 'ذخیره',
    delete: 'حذف',
    edit: 'ویرایش',
    back: 'بازگشت',
    unknown: 'نامشخص',
    yes: 'بله',
    no: 'خیر',
    add: 'افزودن',
    actions: 'عملیات'
  }
} as const

export default fa
export type Translations = typeof fa
