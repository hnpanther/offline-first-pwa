...
- **A recorder's `finished` promise resolves *inside* the manual stop, so a watcher on it re-enters the same handler in the same tick.** `AttachmentFieldInput` routes all three endings — the operator, the duration cap, the byte cap — through one stop handler, which is right. But a manual stop calls `recorder.stop()`, which resolves `finished`, which fires the effect watching it **before** `setRecorder(null)` has run the cleanup that sets `cancelled`; the handler's own `if (!recorder) return` then reads the non-null recorder its closure captured, and `stop()` on an already-finished recorder hands back the same blob. One tap on «پایان ضبط» wrote **two identical rows** — measured in a browser: two 5757-byte rows and a counter of 2 / 1 against a ceiling of 1. Video behaved the same. The guard is `createCaptureGuard()`, a closure held in a ref: React state cannot see a second call in the same tick, which is exactly when this one arrives.

- **A media field's id list must be answered from the device, never subtracted from or appended to the `ids` this render closed over.** Two writes in one tick both read the same stale list, so the second overwrites the first and one row ends up named by nothing. That row is then **invisible and countable at once**: the item list is built from the reference, while the ceiling is counted from `getAttachmentsForEntry`. A one-clip field therefore read as full with nothing on screen to delete — the operator could not record, could not delete, and had no way out. `fieldReferenceFor(deviceRows, currentIds)` is the one answer, used by capture *and* delete, and it **adopts** an unreferenced row rather than dropping it: that adoption is the only route by which a tablet already carrying an orphan is freed, and it happens the moment the field is opened. Regression: `attachmentCapture.test.ts`.

- **A `409` from the delete queue means "not this one, not yet" — skip it, never stop the pass.** The server freezes an approved round's attachments: `DELETE /api/attachments/{id}` answers `409` until somebody withdraws the approval. `drainPendingDeletes` treated everything except `404` and offline as *"leave it queued and stop"*, so one frozen sheet would have held up every unrelated deletion on the device — the wedge gotcha #109 fixed for a different cause. The row still stays queued, because the deletion is still wanted and becomes possible the moment the approval is withdrawn; what changed is that the pass continues past it.<br><br>Worth knowing the refusal is reachable at all: the drain already skips a sheet the device knows is delivered (that branch keeps the server's copy). But a round completed and approved **on the web** while this tablet still holds it as a local draft goes down the server branch, and only learns better on the next bundle. Regression: `attachmentDelete.test.ts` § *keeps a frozen one queued but does not let it hold up the rest of the queue*, which is order-deterministic on purpose — with the ids the other way round a `break` would pass.

- **`npm audit` reports two moderate advisories against `react-router`, and neither is reachable from this app — check that before anyone "fixes" it into a breaking major.** Both are bounded by `>=6.0.0 <7.18.0`, and the remedy `npm audit fix --force` offers is `react-router-dom@7`, a breaking upgrade. Verified against the code rather than the version number:<br><br>**GHSA-337j-9hxr-rhxg** (arbitrary constructor injection via `deserializeErrors()`) is in the **SSR hydration** path. This app renders entirely in the browser — `App.tsx` mounts a plain `BrowserRouter`, and there is no `StaticRouter`, `createStaticHandler`, `renderToString` or `hydrateRoot` anywhere in `src`. The vulnerable function is never called.<br><br>**GHSA-wrjc-x8rr-h8h6** (open redirect via a backslash in `<Link>` / `useNavigate`) *would* be reachable in a client-only SPA, so it was checked target by target rather than dismissed. Every navigation destination is a hardcoded literal or a locally generated id: `postLoginPath()` returns the constant `'/'` (this is the same rule README states as "login never restores a previous user's deep link"), `Sidebar.handleNav` is only ever called with literals or a `path` from a hardcoded array, and the dynamic ones are `/logsheets/${localId}` where the id is a UUID this device minted. **Nothing routes a server- or user-supplied string into navigation.**<br><br>What would make either live: adding SSR, or introducing a `?redirect=`/`?next=` parameter, a deep link restored from storage, or any `navigate()` fed from an API response. If you add one of those, upgrade first. Until then the upgrade is hygiene — worth doing to keep `npm audit` quiet, not worth a rushed major.
